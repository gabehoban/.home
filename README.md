...
```shell
  open https://github.com/gabehoban/dotfiles
```

```shell
  cd ~/Desktop && git clone https://github.com/gabehoban/dotfiles.git && cd dotfiles
```
...
```shell
  ./setup/homebrew.zsh && open -a Dropbox && open -a Lastpass
```
...
**6: Sign in dropbox☁️**  
Sign in and wait for Settings to Sync  
⚠️ Sync only **Settings** file (Downloading everything takes too much time.)

## While waiting for Dropbox to complete ⏳
